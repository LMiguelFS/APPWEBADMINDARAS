...
import { useProducts } from '../../context/ProductContext'; // Importa el contexto de productos unificado
import { useReferenceCRUD } from '../../hooks/useReferenceCRUD'; // Importa el hook genérico de CRUD
import { Event } from '../../types'; // Asegúrate de importar el tipo Event

const AddEvento: React.FC = () => {
  // Obtiene las funciones y opciones relacionadas con eventos del ProductContext
  const { eventOptions, createEvent, updateEvent, deleteEvent } = useProducts();
  const navigate = useNavigate();

  // Utiliza el hook personalizado useReferenceCRUD para gestionar la lógica de eventos
  const {
    items: events, // Renombra 'items' a 'events' para mayor claridad en este componente
    name,
    setName,
    error,
    success,
    ...
    handleDelete,
    handleCancelEdit,
  } = useReferenceCRUD<Event>(
    eventOptions, // Pasa la lista de eventos del contexto
    {
      // Adapta la función 'add': ahora espera un string y lo convierte a { name: string }
      add: async (eventName: string) => await createEvent({ name: eventName }),
      // Adapta la función 'edit': ahora espera un id y un string para el nombre
      edit: async (id: number, eventName: string) => await updateEvent(id, { name: eventName }),
      // remove se mantiene igual ya que solo requiere el id
      remove: async (id: number) => await deleteEvent(id),
    }
  );
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 max-w-5xl mx-auto mt-8 p-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 flex-1 p-6 overflow-auto">
        ...
        {loading ? (
          <div className="text-gray-500">Cargando tipos de evento...</div>
        ) : (
                  ...
                  events.map((event) => (
                    <tr key={event.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 text-center text-sm font-medium">
                        <button
                          onClick={() => handleEdit(event.id)} // handleEdit es proporcionado por useReferenceCRUD
                        ...
                        <button
                          onClick={() => handleDelete(event.id)} // handleDelete es proporcionado por useReferenceCRUD
