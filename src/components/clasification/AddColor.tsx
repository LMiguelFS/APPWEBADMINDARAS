...
import { Color } from '../../types'; // Asegúrate de importar el tipo Color

const AddColor: React.FC = () => {
  // Obtiene las funciones y opciones relacionadas con colores del ProductContext
  const { colorOptions, createColor, updateColor, deleteColor } = useProducts();
  const navigate = useNavigate();

  // Utiliza el hook personalizado useReferenceCRUD para gestionar la lógica de colores
  const {
    items: colors, // Renombra 'items' a 'colors' para mayor claridad en este componente
    name,
    setName,
    error,
    success,
    ...
    handleDelete,
    handleCancelEdit,
  } = useReferenceCRUD<Color>(
    colorOptions, // Pasa la lista de colores del contexto
    {
      // Adapta la función 'add': ahora espera un string y lo convierte a { name: string }
      add: async (colorName: string) => await createColor({ name: colorName }),
      // Adapta la función 'edit': ahora espera un id y un string para el nombre
      edit: async (id: number, colorName: string) => await updateColor(id, { name: colorName }),
      // remove se mantiene igual ya que solo requiere el id
      remove: async (id: number) => await deleteColor(id),
    }
  );

  // Simula un estado de carga, podrías agregar un estado 'loading' al useReferenceCRUD si fuera necesario
  const loading = false; // Por ahora, lo mantenemos simple.
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 max-w-5xl mx-auto mt-8 p-4">
      ...
      {/* Panel de agregar/modificar color */}
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden max-w-md w-full">
        ...
        <div className="px-6 py-5 border-b border-gray-200">
          <h1 className="text-xl font-bold text-gray-900">
            {editingId ? 'Modificar Color' : 'Agregar nuevo Color'}
          </h1>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 flex-1 p-6 overflow-auto">
        ...
        {loading ? (
          <div className="text-gray-500">Cargando colores...</div>
        ) : (
                ...
                {colors.length === 0 ? (
                  ...
                  colors.map((color) => (
                    <tr key={color.id} className="hover:bg-gray-50">
                      <td className="px-4 py-2 text-sm text-gray-900">{color.id}</td>
                      <td className="px-4 py-2 text-sm text-gray-900">{color.name}</td>
                      <td className="px-4 py-2 text-center text-sm font-medium">
                        <button
                          onClick={() => handleEdit(color.id)} // handleEdit es proporcionado por useReferenceCRUD
                        ...
                        <button
                          onClick={() => handleDelete(color.id)} // handleDelete es proporcionado por useReferenceCRUD
