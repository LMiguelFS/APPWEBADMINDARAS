...
import { Forma } from '../../types'; // Asegúrate de importar el tipo Forma

const AddForma: React.FC = () => {
  // Obtiene las funciones y opciones relacionadas con formas del ProductContext
  const { formOptions, createForm, updateForm, deleteForm } = useProducts();
  const navigate = useNavigate();

  // Utiliza el hook personalizado useReferenceCRUD para gestionar la lógica de formas
  const {
    items: formas, // Renombra 'items' a 'formas' para mayor claridad en este componente
    name,
    setName,
    error,
    success,
    ...
    handleDelete,
    handleCancelEdit,
  } = useReferenceCRUD<Forma>(
    formOptions, // Pasa la lista de formas del contexto
    {
      // Adapta la función 'add': ahora espera un string y lo convierte a { name: string }
      add: async (formaName: string) => await createForm({ name: formaName }),
      // Adapta la función 'edit': ahora espera un id y un string para el nombre
      edit: async (id: number, formaName: string) => await updateForm(id, { name: formaName }),
      // remove se mantiene igual ya que solo requiere el id
      remove: async (id: number) => await deleteForm(id),
    }
  );
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 max-w-5xl mx-auto mt-8 p-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 flex-1 p-6 overflow-auto">
        ...
        {loading ? (
          <div className="text-gray-500">Cargando formas...</div>
        ) : (
                ...
                {formas.length === 0 ? (
                  ...
                  formas.map((forma) => (
                    <tr key={forma.id} className="hover:bg-gray-50">
                      <td className="px-4 py-2 text-sm text-gray-900">{forma.id}</td>
                      <td className="px-4 py-2 text-sm text-gray-900">{forma.name}</td>
                      <td className="px-4 py-2 text-center text-sm font-medium">
                        <button
                          onClick={() => handleEdit(forma.id)} // handleEdit es proporcionado por useReferenceCRUD
                        ...
                        <button
                          onClick={() => handleDelete(forma.id)} // handleDelete es proporcionado por useReferenceCRUD
