...
const Reports: React.FC = () => {
  ...
  const totalCost = sales.reduce((sum, sale) => {
    ...
    return sum + (sale.quantity * (product?.cost || 0));
  }, 0);
  ...
  sales.forEach(sale => {
    ...
    if (product) {
      ...
      salesByCategory[category] = (salesByCategory[category] || 0) + (sale.quantity * sale.salePrice);
    }
  });
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${profitMargin > 20 ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'
              }`}>
          ...
          <div className="mt-2 h-1 w-full bg-gray-200 rounded-full">
            <div
              className={`h-1 rounded-full ${profitMargin > 20 ? 'bg-green-500' : 'bg-amber-500'}`}
              style={{ width: `${Math.min(profitMargin * 1.5, 100)}%` }}
            ></div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
            ...
            <p className="text-2xl font-bold text-amber-600">{products.filter(p => p.stock <= 5).length}</p>
          </div>
